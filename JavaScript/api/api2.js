...
export async function buscarTaxonPBDB(nome) {
...
export async function buscarOcorrenciasPBDB(nome) {
  ...
  try {
    const response = await fetch(url);
    if (!response.ok)
      return { erro: `Erro Ocorrências PBDB: ${response.status}` };
...
export async function BuscarImagem(nome) {
  try {
    console.log(nome);
    ...
    const linkImagem = dados.find((imagem) => imagem.nome.toLowerCase() === nome.toLowerCase());

    return linkImagem.imagem || null;
...
// const dados = await BuscarImagem("Allkaruen");
// console.log(dados);
